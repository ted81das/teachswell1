...
abstract class Notification extends Setting {
	...
	public function to_string(): string {
		...
		switch ( $this->frequency ) {
			case 'daily':
				...
				return sprintf( __( '%1$s at %2$s', 'defender-security' ), ucfirst( $this->frequency ), $date->format( 'h:i A' ) );
			case 'weekly':
				/* translators: 1: Notification sending frequency, 2: Day of the week, 3: Time of a day. */
				return sprintf( __( '%1$s on %2$s at %3$s', 'defender-security' ), ucfirst( $this->frequency ), ucfirst( $this->day ), $date->format( 'h:i A' ) );
			case 'monthly':
			default:
				/* translators: 1: Notification sending frequency, 2: Day of the month, 3: Time of a day. */
				return sprintf( __( '%1$s/%2$d, %3$s', 'defender-security' ), ucfirst( $this->frequency ), $this->day_n, $date->format( 'h:i A' ) );
		}
	}
	...
	public function get_next_run_as_string( bool $for_hub = false ) {
		if ( 'notification' === $this->type ) {
			return $for_hub ? false : __( 'Never', 'defender-security' );
		}
		...
		if ( $for_hub ) {
			...
			if ( $this->check_active_status() ) {
				...
				return __( 'Never', 'defender-security' );
			}
		}
	}
	...
	protected function after_validate(): void {
		foreach ( $this->out_house_recipients as $recipient ) {
			...
			if ( ! filter_var( $recipient['email'], FILTER_VALIDATE_EMAIL ) ) {
				...
				$this->errors[] = sprintf( __( 'Email %s is invalid format', 'defender-security' ), $recipient['email'] );
			}
		}
	}
	...
	public function export(): array {
		...
		if ( isset( $l10n['defender-security'] ) ) {
			$data['title'] = __( $data['title'], 'defender-security' );
			$data['description'] = __( $data['description'], 'defender-security' );
		}
