...
class Firewall_Notification extends \WP_Defender\Model\Notification {
	...
	private function send_to_user( string $email, string $name, Lockout_Log $model, string $template, object $service ): void {
		...
		if ( 'login-lockout' === $template ) {
			...
			$subject = sprintf( __( 'Login lockout alert for %s', 'defender-security' ), $network_site_url );// phpcs:ignore
			...
			if ( false !== strpos( $model->log, '2fa attempts' ) ) {
				...
				$time_limit = $component->get_time_limit() . __( ' seconds', 'defender-security' );
				$type = '2fa';
			} else {
			...
			$text = __( 'The host <strong>%1$s</strong> has been locked out of <a href="%2$s">%3$s</a> due to more than <strong>%4$s</strong> failed %5$s attempts. %6$s', 'defender-security' );// phpcs:ignore
			if ( 'permanent' === $settings->lockout_type ) {
				$string = __( 'Accordingly, the host has been permanently banned.', 'defender-security' );
			} else {
				$string = sprintf(
			...
			$subject = sprintf( __( 'User Agent lockout alert for %s', 'defender-security' ), $network_site_url );// phpcs:ignore
			$text = sprintf(
			/* translators: 1: User agent, 2: Site URL, 3: Site URL. */
				__( 'The <strong>%1$s</strong> has been locked out of <a href="%2$s">%3$s</a>.', 'defender-security' ),
			...
			/* translators: %s: Site URL. */
			$subject = sprintf( __( '404 lockout alert for %s', 'defender-security' ), $network_site_url );// phpcs:ignore
			...
			$text = __( 'The host <strong>%1$s</strong> has been locked out of <a href="%2$s">%3$s</a> due to more than <strong>%4$s</strong> 404 requests for the file <strong>%5$s</strong>. %6$s', 'defender-security' );// phpcs:ignore
			if ( 'permanent' === $settings->lockout_type ) {
				$string = __( 'Accordingly, the host has been permanently banned.', 'defender-security' );
			} else {
				/* translators: 1: Duration, 2: Duration Unit. */
				$string = sprintf(
					__( 'They have been locked out for <strong>%1$s %2$s</strong>.', 'defender-security' ),
	...
	public function labels(): array {
		return [
			'notification' => __( 'Firewall - Notification', 'defender-security' ),
			'login_lockout_notification' => __( 'Login Protection Lockout', 'defender-security' ),
			'ip_lockout_notification' => __( '404 Detection Lockout', 'defender-security' ),
			'ua_lockout_notification' => __( 'User Agent Lockout', 'defender-security' ),
			'notification_subscribers' => __( 'Recipients', 'defender-security' ),
			'cooldown_enabled' => __( 'Limit email notifications for repeat lockouts', 'defender-security' ),
			'cooldown_number_lockout' => __( 'Repeat Lockouts Threshold', 'defender-security' ),
			'cooldown_period' => __( 'Repeat Lockouts Period', 'defender-security' ),
		];
	}
