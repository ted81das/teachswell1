...
class Backup_Codes extends Two_Factor_Provider {
	...
	public function init_provider( WP_User $user ): void {
		...
		$this->description = empty( $this->is_activated )
			? __( 'Generate non-expirable backup codes that can be used to log in once.', 'defender-security' )
			: __( 'Each backup code can only be used to log in once.', 'defender-security' );
	}
	...
	public function get_label(): string {
		return __( 'Backup Codes', 'defender-security' );
	}
	...
	public function authentication_form(): void {
		...
		<p class="wpdef-2fa-text">
			<?php echo __( 'Enter one of your recovery codes to log in to your account.', 'defender-security' ); ?>
		</p>
				...
				<span>
					...
					echo wp_kses(
						...
						/* translators: %s: URL to regenerate code */
							__( 'You\'ve used all generated backup codes. Please <a href="%s">generate</a> new codes accordingly.', 'defender-security' ),
	...
	public function user_options( WP_User $user ): void {
		...
		if ( empty( $this->is_activated ) ) {
			$button_text = __( 'Generate Backup Codes', 'defender-security' );
			$class = 'hidden';
			$count = 0;
			$show_notice = 'no';
		} else {
	...
	protected function validate_code( WP_User $user, string $code ) {
		...
		return new WP_Error(
			'opt_fail',
			empty( $lockout_message ) ? __( 'ERROR: Invalid verification code.', 'defender-security' ) : $lockout_message
		);
	}
	...
	public function validate_authentication( WP_User $user ) {
		...
		if ( false === $backup_code ) {
			return new WP_Error(
				'opt_fail',
				__( 'ERROR: Invalid backup code.', 'defender-security' )
			);
		}
