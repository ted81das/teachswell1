...
class Main_Setting extends Controller {
	...
	public function apply_config( Request $request ) {
		...
		if ( $restore_result ) {
			...
			$message .= '<br/>' . sprintf(
				...
				__(
					'Because of some security recommendations get applied, You will now need to <a href="%s"><strong>re-login</strong></a>.',
					'defender-security'
				),
				$login_url
			);
	...
	private function apply_config_recommendations_error_message(): Response {
		$message = sprintf(
		/* translators: %s: Link. */
			__( 'There was an issue with applying some of the tweaks from the <strong>Recommendations</strong> tab because we cannot make changes to your <strong>wp-config.php</strong> file. Please see our %s to apply the changes manually.', 'defender-security' ),
