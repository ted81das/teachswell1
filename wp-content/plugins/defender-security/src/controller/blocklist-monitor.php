...
class Blocklist_Monitor extends Controller {
	...
	public function toggle_blacklist_status( Request $request ) {
		...
		if ( ! $this->is_pro() ) {
			return new Response(
				false, [
					'message' => __( 'A WPMU DEV subscription is required for blocklist monitoring.', 'defender-security' ),
				]
			);
		}
	...
	public function config_strings( $config, $is_pro ): array {
		if ( $is_pro ) {
			$strings = $config['enabled'] ? [ __( 'Active', 'defender-security' ) ] : [ __( 'Inactive', 'defender-security' ) ];
		} else {
			$strings = [
				sprintf(
				/* translators: %s: Html for Pro-tag. */
	...
	public function labels(): array {
		return [
			'enabled' => __( 'Blocklist Monitor', 'defender-security' ),
			'status' => __( 'Status', 'defender-security' ),
		];
	}
}
